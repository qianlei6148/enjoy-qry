<template>
	<view class="pages">
		...
		<!-- 搜索功能 -->
		<view class="uni-search-box cu-bar bg-gradual-pink shadow-blur" >
			...
			<uni-list class="uni-list" :border="false" :style="{height:listHight}">
				...
				<uni-list-item :to="'/pages/list/detail?id='+item._id+'&title='+item.title" v-for="(item,index) in data"
					...
					<template v-slot:body>
						<view class="main">
							<!-- <uni-list-item thumbSize="sm" :thumb="item.category_id[0].icon"></uni-list-item> -->
							<view class="info">
								<view class="" style="width: 50rpx;">
									<image :src="item.category_id[0].icon" class="uni-list-item__icon-img uni-list--sm author" ></image>
								</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			colList(){
				return [
					db.collection('opendb-news-articles').where(this.where).field('avatar,category_id,title,last_modify_date,user_id').getTemp(),
					db.collection('uni-id-users').field('_id,username').getTemp(),
					db.collection('opendb-news-categories').field('_id,icon').getTemp()
				]
			}
		},
		data() {
